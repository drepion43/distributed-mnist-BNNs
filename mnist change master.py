...
def train(gpu, args):
    ...
    model.load_state_dict(torch.load(dir + 'model_state_dict.pt'))
    checkpoint=torch.load(dir+'all.tar')
...
if __name__ == '__main__':
    ...
    file_name='mnist_cnn_param'
    dir=os.path.join(cur_dir,ckpt_dir)
    os.makedirs(dir,exist_ok=True)
    file_path=os.path.join(dir,file_name)

    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server_address = ('192.168.0.14', 10000)
    sock.bind(server_address)
    sock.listen(1)
    conn, addr = s.accept()
    fileName = conn.recv(1024)
    fileName = fileName.decode()
    conn.sendall(getFileSize(fileName, dir).encode())
